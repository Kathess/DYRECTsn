...
from scenarios.carnet.car_topology import car_topology
from scenarios.carnet.car_flows import car_flow_updates, car_flows
from scenarios.carnet.car_gcls import set_car_gcls
...
if __name__ == "__main__":
    ...
    """
    # filepath = "carnet/results/" + time.strftime("%Y%m%d-%H%M%S") + ".txt"
    ...
    nrCBSqueues = 2  # should be between 1 and 6 in TSN
    graph, bridges, tsn_devices = car_topology(linkrate_backbone=Decimal(1.0e10), linkrate_nodes=Decimal(1.0e9), output=False,
                                                                nrCBSqueues=nrCBSqueues)
    routing = 1  # 1=shortest path, 2=balanced network load

    flow_operations = car_flow_updates()
    flow_list = car_flows()

    max_best_effort_frame_size = Decimal('12272')
    ...
    individual_delays = False ## TODO try with individual delay budgets

    # Other parameters which might be interesting:
    delay_bound_limits = (20, 500)  # min. and max. possible delay bound per queue (in x10 microseconds)
    ...
    if skip:
        ...
        if not individual_delays:
            best_delay_bounds = [20,20]
        else:
            best_delay_bounds = [132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115, 132, 115]
            # Best fitness:  0.99
    ...
    #         if bridge != bridge2:
    #             if (bridge, bridge2) in best_delay_bounds:
    #                 # set delay bounds to previous value / 10
    #                 best_delay_bounds[(bridge, bridge2)] = [Decimal('0.000132'), Decimal('0.000115')]
    #     if (bridge, 'adas') in best_delay_bounds:
    #         best_delay_bounds[(bridge, 'adas')] = [Decimal('0.000132'), Decimal('0.000115')]
    # print(best_delay_bounds)
    ...
    if individual_delays:
        graph_state = init_topology(graph=graph, delay_per_queue=best_delay_bounds,
                                    ...
                                    output=True, constant_priority=constant_priority)
    else:
    ...
    # period = 1000 microsec., gate open time = 50 microsec.
    set_car_gcls(graph_state)
    # ------------------------------------------------------------------------------
    ...
    avgCalc = 0
    numCalc = 0
    for operation, flow in flow_operations:
        # measure time of the add flow operation
        start = time.time()
        if operation == 'add':
            results = add_flow(flow=flow, graph_state=graph_state, routing=routing)
        elif operation == 'remove':
            results = remove_flow(flow=flow, graph_state=graph_state)
        else:
        ...
        print(f"----- {i + 1}. Flow Operation -- {operation} -----")
    ...
    print("Unsuccessful reservations: ", unsuccessful_reservations)
    print("Total flow operations:", i)
    ...
    print("Min. Calculation Time: ", minCalc)
    print("Max. Calculation Time: ", maxCalc)
    print("Avg. Calculation Time: ", avgCalc / numCalc)
    print("----- End Online Admission ----")
